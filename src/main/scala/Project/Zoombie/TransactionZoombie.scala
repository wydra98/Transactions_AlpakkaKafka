...
object TransactionZoombie extends App {
  ...
  val producer = Source(listOfProduct)
    ...
    .map { product =>
      ...
      ProducerMessage.multi(
        immutable.Seq(
          new ProducerRecord[String, String]("producerToTransaction", product.toString),
          new ProducerRecord[String, String]("producerToTransactionZoombie", product.toString)
        )
      )
    }
  ...
  val transaction = Transactional
    ...
    .map { msg =>
      ...
      println(f"${GREEN}ReSend -> ID: ${product(0)}%-3s| name: ${product(1)}%-8s|" +
        f" amount: ${product(2)}%-2s| price: ${product(3)}%-6s| offest: ${msg.partitionOffset.offset} | noZoombie${RESET}")
      ...
      ProducerMessage.single(new ProducerRecord("transactionToConsumerZoombie", msg.record.key, msg.record.value),
        msg.partitionOffset)
    }
  ...
  /** TRANSAKCJA ZOOMBIE PRZESYŁAJĄCA DANE DO KONSUMENTA */
  val transactionZoombie = Transactional
    .source(ProjectProperties.consumerSettings_1, Subscriptions.topics("producerToTransactionZoombie"))
    .map { msg =>
      val product = msg.record.value().split(",")
      println(f"${MAGENTA}ReSend -> ID: ${product(0)}%-3s| name: ${product(1)}%-8s|" +
        f" amount: ${product(2)}%-2s| price: ${product(3)}%-6s| offest: ${msg.partitionOffset.offset} | zoombie${RESET}")
  ...
  val consumer = {
    Consumer
      .plainSource(ProjectProperties.consumerSettings_2, Subscriptions.topics("transactionToConsumerZoombie"))
      .map((msg) => {
  ...
  val totalSource = Source.combine(producer, transaction, transactionZoombie, consumer)(Merge(_))
